...
#define GENERATE_OUTPUT_FUNC_PROTOS(o)                                         \
  static int output_headers_##o(corsaro_t *corsaro, corsaro_file_t *file,      \
                                corsaro_header_t *header);                     \
  static int output_interval_start_##o(                                        \
    corsaro_t *corsaro, corsaro_file_t *file, corsaro_interval_t *int_start);  \
  static int output_interval_end_##o(corsaro_t *corsaro, corsaro_file_t *file, \
                                     corsaro_interval_t *int_end);             \
                                ...
                                corsaro_header_t *header)
{
  ...
  if (header == NULL) {
    while ((tmp = corsaro_plugin_next(corsaro->plugin_manager, tmp)) != NULL) {
      bytes_out +=
        corsaro_file_printf(corsaro, file, "# CORSARO_PLUGIN %s\n", tmp->name);
    }
  } else {
    for (i = 0; i < header->plugin_cnt; i++) {
      bytes_out += corsaro_file_printf(
        corsaro, file, "# CORSARO_PLUGIN %s\n",
        corsaro_plugin_get_name_by_magic(corsaro->plugin_manager,
                                 ...
                                 corsaro_header_t *header)
{
  ...
  *ptr = CORSARO_MID_VERSION;
  ...
  if (corsaro->uridata != NULL) {
    ...
    if (corsaro_file_write(corsaro, file, corsaro->uridata, tmp_len) !=
        tmp_len) {
...
static int output_interval_start_ascii(corsaro_t *corsaro, corsaro_file_t *file,
                                       corsaro_interval_t *int_start)
{
  return corsaro_file_printf(corsaro, file, "# CORSARO_INTERVAL_START %d %ld\n",
...
static int output_interval_end_ascii(corsaro_t *corsaro, corsaro_file_t *file,
                                     corsaro_interval_t *int_end)
{
  return corsaro_file_printf(corsaro, file, "# CORSARO_INTERVAL_END %d %ld\n",
                             int_end->number, int_end->time);
}

static int write_interval_header_binary(corsaro_t *corsaro,
                                        corsaro_file_t *file,
...
static int output_interval_start_binary(corsaro_t *corsaro,
                                        corsaro_file_t *file,
                                        corsaro_interval_t *int_start)
{
...
static int output_plugin_start_ascii(corsaro_t *corsaro, corsaro_file_t *file,
                                     corsaro_plugin_t *plugin)
{
  return corsaro_file_printf(corsaro, file, "# CORSARO_PLUGIN_DATA_START %s\n",
                             plugin->name);
}
...
static int output_plugin_end_ascii(corsaro_t *corsaro, corsaro_file_t *file,
                                   corsaro_plugin_t *plugin)
{
  return corsaro_file_printf(corsaro, file, "# CORSARO_PLUGIN_DATA_END %s\n",
                             plugin->name);
}
                                      ...
                                      corsaro_plugin_t *plugin)
{
  corsaro_plugin_data_t data = {
    htonl(CORSARO_MAGIC), htonl(CORSARO_MAGIC_DATA), htonl(plugin->magic),
  };

  if (corsaro_file_write(corsaro, file, &data, sizeof(corsaro_plugin_data_t)) !=
                                 ...
                                 corsaro_trailer_t *trailer)
{
  ...
  bytes_out += corsaro_file_printf(
    corsaro, file, "# CORSARO_PACKETCNT %" PRIu64 "\n", corsaro->packet_cnt);
  if (acnt != UINT64_MAX) {
    bytes_out += corsaro_file_printf(
      corsaro, file, "# CORSARO_ACCEPTEDCNT %" PRIu64 "\n", acnt);
  }
  if (dcnt != UINT64_MAX) {
    bytes_out += corsaro_file_printf(
      corsaro, file, "# CORSARO_DROPPEDCNT %" PRIu64 "\n", dcnt);
  }
  ...
  bytes_out +=
    corsaro_file_printf(corsaro, file, "# CORSARO_FINALTIME %ld\n", ts.tv_sec);
                                  ...
                                  corsaro_trailer_t *trailer)
{
  ...
  if (corsaro_file_write(corsaro, file, &buff[0],
...
static int validate_header_static(corsaro_header_t *h)
{
  ...
  if (h->corsaro_magic != CORSARO_MAGIC || h->magic != CORSARO_MAGIC_HEADER) {
    return 0;
  }
  return 1;
}
...
static int validate_plugin_data(corsaro_plugin_data_t *pd)
{
  ...
  /* do some sanity checking on the interval */
  if (pd->corsaro_magic != CORSARO_MAGIC || pd->magic != CORSARO_MAGIC_DATA) {
    return 0;
  }
  return 1;
}
...
static int validate_trailer(corsaro_trailer_t *t)
{
  ...
  /* do some sanity checking on the interval */
  if (t->corsaro_magic != CORSARO_MAGIC || t->magic != CORSARO_MAGIC_TRAILER) {
    return 0;
  }
  return 1;
}

off_t read_plugin_data(corsaro_in_t *corsaro, corsaro_file_in_t *file,
                                        ...
                                        corsaro_interval_t *interval)
{
  return corsaro_io_prepare_file_full(corsaro, plugin_name, interval,
                                      corsaro->output_mode, corsaro->compress,
                                      corsaro->compress_level, O_CREAT);
}
                                             ...
                                             int compress_level, int flags)
{
  ...
  if ((f = corsaro_file_open(corsaro, outfileuri, mode, compress,
                             compress_level, flags)) == NULL) {
    corsaro_log(__func__, corsaro, "could not open %s for writing", outfileuri);
    return NULL;
  }
                             ...
                             corsaro_header_t *header)
{
  ...
  for (i = 0; i < header->plugin_cnt; i++) {
    fprintf(stdout, "# CORSARO_PLUGIN %s\n",
            corsaro_plugin_get_name_by_magic(plugin_manager,
                                             header->plugin_magics[i]));
  }
}
                               ...
                               corsaro_trailer_t *trailer)
{
  assert(CORSARO_FILE_MODE(file) == CORSARO_FILE_MODE_ASCII ||
         CORSARO_FILE_MODE(file) == CORSARO_FILE_MODE_BINARY);
  return output_funcs[CORSARO_FILE_MODE(file)].trailers(corsaro, file, trailer);
}
...
{
  fprintf(stdout, "# CORSARO_INTERVAL_START %d %" PRIu32 "\n",
          int_start->number, int_start->time);
}
...
{
  fprintf(stdout, "# CORSARO_INTERVAL_END %d %" PRIu32 "\n", int_end->number,
          int_end->time);
}
 ...
 * @todo add code to \link corsaro_file_open \endlink that creates a special
 * 'stdout' file
 */
                              ...
                              corsaro_in_record_t *record)
{
  ...
  case CORSARO_IN_RECORD_TYPE_IO_INTERVAL_START:
    return corsaro_io_write_interval_start(
      corsaro, file, (corsaro_interval_t *)record->buffer);
    break;
                            ...
                            corsaro_in_record_t *record)
{
  ...
  case CORSARO_IN_RECORD_TYPE_IO_HEADER:
    corsaro_io_print_header(plugin_manager, (corsaro_header_t *)record->buffer);
    break;
                             ...
                             corsaro_in_record_t *record)
{
  ...
  if (header->traceuri_len == 0) {
    ...
    if ((bread += corsaro_io_read_bytes_offset(corsaro, record, offset,
                                               header->traceuri_len)) !=
        (bsbread += header->traceuri_len)) {
      corsaro_log_in(__func__, corsaro, "failed to read traceuri from file");
      *record_type = CORSARO_IN_RECORD_TYPE_NULL;
      return bread;
    }
  ...
  if ((bread += corsaro_io_read_bytes_offset(
         corsaro, record,
         CORSARO_IO_HEADER_FIXED_BYTE_LEN + sizeof(uint16_t) +
           sizeof(uint8_t *),
         sizeof(uint16_t))) != (bsbread += sizeof(uint16_t))) {
    corsaro_log_in(__func__, corsaro, "failed to read plugin count from file");
    *record_type = CORSARO_IN_RECORD_TYPE_NULL;
    return bread;
  }
  ...
  if (header->plugin_cnt == 0) {
    ...
    if ((bread += corsaro_io_read_bytes_offset(
           corsaro, record, offset, sizeof(uint32_t) * header->plugin_cnt)) !=
                              ...
                              corsaro_in_record_t *record)
{
  ...
  if ((bytes_read =
         corsaro_io_read_bytes(corsaro, record, sizeof(corsaro_trailer_t))) !=
...
off_t corsaro_io_read_bytes_offset(corsaro_in_t *corsaro,
                                   corsaro_in_record_t *record, off_t offset,
                                   off_t len)
{
  /* fix this with a realloc later? */
  assert(record->buffer_len >= offset + len);

  return corsaro_file_rread(corsaro->file, (record->buffer) + offset, len);
}
