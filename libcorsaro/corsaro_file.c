...
corsaro_file_t *corsaro_file_open(corsaro_t *corsaro, const char *filename,
                                  ...
                                  int compress_level, int flags)
{
  ...
  case CORSARO_FILE_MODE_BINARY:
    if ((f->wand_io = wandio_wcreate(filename, compress_type, compress_level,
                                     flags)) == NULL) {
...
off_t corsaro_file_write(corsaro_t *corsaro, corsaro_file_t *file,
                         const void *buffer, off_t len)
{
...
off_t corsaro_file_write_packet(corsaro_t *corsaro, corsaro_file_t *file,
                                libtrace_packet_t *packet)
{
  ...
  case CORSARO_FILE_MODE_BINARY:
    assert(file->wand_io != NULL);
    if ((pkt_buf = trace_get_packet_buffer(packet, &linktype, NULL)) == NULL) {
...
off_t corsaro_file_printf(corsaro_t *corsaro, corsaro_file_t *file,
                          const char *format, ...)
{
  va_list ap;
...
off_t corsaro_file_rread_packet(corsaro_file_in_t *file,
                                libtrace_packet_t *packet, uint16_t len)
{
  uint8_t *pktbuf;
  ...
  case CORSARO_FILE_MODE_BINARY:
    ...
    trace_construct_packet(packet, TRACE_TYPE_ETH, pktbuf, len);
    return len;
    break;
