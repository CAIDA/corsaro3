...
static int start_interval(corsaro_t *corsaro, struct timeval int_start)
{
  ...
  if (corsaro->logfile_disabled == 0 && corsaro->logfile == NULL) {
    ...
    if (corsaro->interval_start.number == 0) {
      ...
      corsaro_log(__func__, corsaro, "now logging to file"
#ifdef DEBUG
                                     " (and stderr)"
#endif
                  );
  ...
  if (corsaro->global_file_disabled == 0 && corsaro->global_file == NULL) {
    if ((corsaro->global_file =
           corsaro_io_prepare_file(corsaro, CORSARO_IO_GLOBAL_NAME,
...
static int end_interval(corsaro_t *corsaro, struct timeval int_end)
{
  ...
  if (is_meta_rotate_interval(corsaro)) {
    if (corsaro->global_file != NULL) {
      /* write headers to the global file */
      if (corsaro_io_write_trailer(corsaro, corsaro->global_file, NULL) <= 0) {
        corsaro_log(__func__, corsaro, "could not write global trailers");
        corsaro_free(corsaro);
        return -1;
      }
      corsaro_file_close(corsaro, corsaro->global_file);
...
static corsaro_in_t *corsaro_in_init(const char *corsarouri)
{
  ...
  if ((e->uridata = strdup(corsarouri)) == NULL) {
    corsaro_log_in(__func__, e, "could not duplicate uri string (no memory?)");
    goto err;
  }
...
static inline int process_packet(corsaro_t *corsaro, corsaro_packet_t *packet)
{
  ...
  while ((tmp = corsaro_plugin_next(corsaro->plugin_manager, tmp)) != NULL) {
    ...
    if (tmp->process_packet(corsaro, packet) < 0) {
      corsaro_log(__func__, corsaro, "%s failed to process packet", tmp->name);
      return -1;
    }
#ifdef WITH_PLUGIN_TIMING
                                     ...
                                     corsaro_flowtuple_class_start_t *class)
{
  ...
  while ((tmp = corsaro_plugin_next(corsaro->plugin_manager, tmp)) != NULL) {
    if (tmp->process_flowtuple_class_start != NULL &&
      ...
      corsaro_log(__func__, corsaro,
                  "%s failed to process flowtuple class start", tmp->name);
      return -1;
    }
  }
                                   ...
                                   corsaro_flowtuple_class_end_t *class)
{
  ...
  while ((tmp = corsaro_plugin_next(corsaro->plugin_manager, tmp)) != NULL) {
    if (tmp->process_flowtuple_class_end != NULL &&
      ...
      corsaro_log(__func__, corsaro, "%s failed to process flowtuple class end",
                  tmp->name);
      return -1;
    }
  }
...
static int is_plugin_data_or_interval(corsaro_in_t *corsaro)
{
  ...
  len = corsaro_file_rpeek(corsaro->file, buffer, sizeof(buffer));
  if (len < sizeof(corsaro_plugin_data_t) && len < sizeof(corsaro_interval_t)) {
    corsaro_log_in(__func__, corsaro, "invalid corsaro global file");
    return -1;
  }
...
static int is_trailer_or_interval(corsaro_in_t *corsaro)
{
  ...
  /* we need to peek and see if there is any plugin data */
  len = corsaro_file_rpeek(corsaro->file, buffer, sizeof(buffer));
  if (len < sizeof(corsaro_trailer_t) && len < sizeof(corsaro_interval_t)) {
    corsaro_log_in(__func__, corsaro, "invalid corsaro global file");
    return -1;
  }
                         ...
                         corsaro_in_record_t *record)
{
  ...
  case CORSARO_IN_RECORD_TYPE_IO_HEADER:
    ...
    bytes_read =
      corsaro_io_read_header(corsaro, corsaro->file, record_type, record);
    if (bytes_read > 0) {
  ...
  case CORSARO_IN_RECORD_TYPE_IO_PLUGIN_START:
    /* ask the io subsystem to read it for us */
    bytes_read =
      corsaro_io_read_plugin_start(corsaro, corsaro->file, record_type, record);
    if (bytes_read == sizeof(corsaro_plugin_data_t)) {
      ...
      corsaro_log_in(__func__, corsaro, "failed to read plugin data start");
      *record_type = CORSARO_IN_RECORD_TYPE_NULL;
    }
    break;
  ...
  case CORSARO_IN_RECORD_TYPE_INTERNAL_REDIRECT:
    ...
    bytes_read =
      corsaro->plugin->read_global_data_record(corsaro, record_type, record);
  ...
  case CORSARO_IN_RECORD_TYPE_IO_PLUGIN_END:
    /* ask the io subsystem to read it for us */
    bytes_read =
      corsaro_io_read_plugin_end(corsaro, corsaro->file, record_type, record);
    if (bytes_read == sizeof(corsaro_plugin_data_t)) {
  ...
  case CORSARO_IN_RECORD_TYPE_IO_TRAILER:
    /* ask the io subsystem to read it for us */
    bytes_read =
      corsaro_io_read_trailer(corsaro, corsaro->file, record_type, record);
                                    ...
                                    corsaro_interval_align_t align)
{
  ...
  corsaro_log(__func__, corsaro, "setting interval alignment to %d", align);
...
void corsaro_set_interval(corsaro_t *corsaro, unsigned int i)
{
  ...
  corsaro_log(__func__, corsaro, "setting interval length to %d", i);
...
void corsaro_set_output_rotation(corsaro_t *corsaro, int intervals)
{
  ...
  corsaro_log(__func__, corsaro, "setting output rotation after %d interval(s)",
              intervals);
...
void corsaro_set_meta_output_rotation(corsaro_t *corsaro, int intervals)
{
  ...
  corsaro_log(__func__, corsaro,
              "setting meta output rotation after %d intervals(s)", intervals);
...
int corsaro_is_rotate_interval(corsaro_t *corsaro)
{
  ...
  if (corsaro->output_rotate == 0) {
    return 0;
  } else if ((corsaro->interval_start.number + 1) % corsaro->output_rotate ==
             0) {
    return 1;
  } else {
    return 0;
...
int corsaro_set_traceuri(corsaro_t *corsaro, char *uri)
{
  ...
  if (corsaro->started != 0) {
    corsaro_log(__func__, corsaro, "trace uri can only be set before "
  ...
  if (corsaro->uridata != NULL) {
    ...
    corsaro_log(__func__, corsaro, "setting trace uri to %s", uri);
  }
...
uint64_t corsaro_get_accepted_packets(corsaro_t *corsaro)
{
  return corsaro->accepted_pkts == UINT64_MAX
           ? UINT64_MAX
           : trace_get_accepted_packets(corsaro->trace) -
               corsaro->accepted_pkts;
}
...
int corsaro_set_monitorname(corsaro_t *corsaro, char *name)
{
  ...
  if (corsaro->started != 0) {
    corsaro_log(__func__, corsaro, "monitor name can only be set before "
                                   "corsaro_start_output is called");
    return -1;
  }

  if (corsaro->monitorname != NULL) {
    corsaro_log(__func__, corsaro, "updating monitor name from %s to %s",
                corsaro->monitorname, name);
  } else {
    corsaro_log(__func__, corsaro, "setting monitor name to %s", name);
  }
...
int corsaro_per_packet(corsaro_t *corsaro, libtrace_packet_t *ltpacket)
{
  ...
  if (corsaro->packet_cnt == 0) {
    ...
    if (corsaro->interval_align == CORSARO_INTERVAL_ALIGN_YES) {
      corsaro->next_report =
        (corsaro->next_report / corsaro->interval) * corsaro->interval;
    }
  }
  ...
  while (corsaro->interval >= 0 &&
         (uint32_t)ts.tv_sec >= corsaro->next_report) {
                       ...
                       corsaro_in_record_t *record)
{
  ...
  if (type == CORSARO_IN_RECORD_TYPE_IO_INTERVAL_START) {
    ...
    flowtuple = (corsaro_flowtuple_t *)corsaro_in_get_record_data(record);
    ...
    class_end =
      (corsaro_flowtuple_class_end_t *)corsaro_in_get_record_data(record);
...
int corsaro_start_input(corsaro_in_t *corsaro)
{
  ...
  while (corsaro->plugin == NULL &&
    ...
    if (p->probe_magic(corsaro, corsaro->file) == 1) {
      corsaro_log_in(__func__, corsaro,
                     "%s plugin selected to read %s (using magic)", p->name,
                     corsaro->uridata);
      corsaro->plugin = p;
    }
  }
