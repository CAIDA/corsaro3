...
static void usage(corsaro_t *corsaro)
{
  ...
  fprintf(stderr, "plugin usage: %s -p provider [-p \"provider arg1...argn\"]\n"
...
static int parse_args(corsaro_t *corsaro)
{
  ...
  for (i = 0; i < provider_names_cnt; i++) {
    ...
    if ((provider = ipmeta_get_provider_by_name(state->ipmeta,
    ...
    if (ipmeta_enable_provider(state->ipmeta, provider, provider_arg_ptr,
                               is_default) != 0) {
...
int corsaro_ipmeta_probe_magic(corsaro_in_t *corsaro, corsaro_file_in_t *file)
{
  /* this writes no files! */
  return 0;
}
...
int corsaro_ipmeta_init_output(corsaro_t *corsaro)
{
  ...
  if ((state = malloc_zero(sizeof(struct corsaro_ipmeta_state_t))) == NULL) {
    corsaro_log(__func__, corsaro, "could not malloc corsaro_maxmind_state_t");
    return -1;
  }
...
off_t corsaro_ipmeta_read_global_data_record(
  corsaro_in_t *corsaro, corsaro_in_record_type_t *record_type,
  corsaro_in_record_t *record)
{
...
int corsaro_ipmeta_end_interval(corsaro_t *corsaro, corsaro_interval_t *int_end)
{
  /* we don't care */
  return 0;
}
...
int corsaro_ipmeta_process_packet(corsaro_t *corsaro, corsaro_packet_t *packet)
{
...
int corsaro_ipmeta_process_flowtuple_class_start(
...
int corsaro_ipmeta_process_flowtuple_class_end(
