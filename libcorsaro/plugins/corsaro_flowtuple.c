...
KHASH_INIT(sixt, corsaro_flowtuple_t *, char, 0, corsaro_flowtuple_hash_func,
...
{
  fprintf(stderr, "plugin usage: %s [-s]\n"
...
static int binary_dump(corsaro_t *corsaro, corsaro_flowtuple_class_type_t dist)
{
  ...
  if (corsaro_file_write(corsaro, STATE(corsaro)->outfile, &hbuf[0], 10) !=
      10) {
  ...
  if (kh_size(h) > 0) {
    if (state->sort_enabled == CORSARO_FLOWTUPLE_SORT_ENABLED) {
      ...
      for (j = 0; j < kh_size(h); j++) {
        if (corsaro_file_write(corsaro, STATE(corsaro)->outfile, sorted_keys[j],
                               CORSARO_FLOWTUPLE_BYTECNT) !=
            CORSARO_FLOWTUPLE_BYTECNT) {
          corsaro_log(__func__, corsaro, "could not write flowtuple to file");
          return -1;
        }
      ...
      for (i = kh_begin(h); i != kh_end(h); ++i) {
        if (kh_exist(h, i)) {
          if (corsaro_file_write(corsaro, STATE(corsaro)->outfile, kh_key(h, i),
                                 CORSARO_FLOWTUPLE_BYTECNT) !=
              CORSARO_FLOWTUPLE_BYTECNT) {
            corsaro_log(__func__, corsaro, "could not write flowtuple to file");
            return -1;
                            ...
                            corsaro_in_record_t *record)
{
  ...
  if ((bytes_read = corsaro_io_read_bytes(
         corsaro, record, sizeof(corsaro_flowtuple_class_start_t))) !=
      sizeof(corsaro_flowtuple_class_start_t)) {
    *record_type = CORSARO_IN_RECORD_TYPE_NULL;
    return bytes_read;
  }

  if (validate_class_start((corsaro_flowtuple_class_start_t *)record->buffer) !=
      1) {
  ...
  *record_type = CORSARO_IN_RECORD_TYPE_FLOWTUPLE_CLASS_START;
  STATE_IN(corsaro)->tuple_total =
    ((corsaro_flowtuple_class_start_t *)record->buffer)->count;
                          ...
                          corsaro_in_record_t *record)
{
  ...
  if ((bytes_read = corsaro_io_read_bytes(
         corsaro, record, sizeof(corsaro_flowtuple_class_end_t))) !=
      sizeof(corsaro_flowtuple_class_end_t)) {
    *record_type = CORSARO_IN_RECORD_TYPE_NULL;
    return bytes_read;
  }

  if (validate_class_end((corsaro_flowtuple_class_end_t *)record->buffer) !=
      1) {
  ...
  *record_type = CORSARO_IN_RECORD_TYPE_FLOWTUPLE_CLASS_END;
  STATE_IN(corsaro)->expected_type =
    (((corsaro_flowtuple_class_end_t *)record->buffer)->class_type ==
                          ...
                          corsaro_in_record_t *record)
{
  ...
  if ((bytes_read =
         corsaro_io_read_bytes(corsaro, record, sizeof(corsaro_flowtuple_t))) !=
                                  ...
                                  corsaro_file_in_t *file)
{
  ...
  if (len >= CORSARO_IO_INTERVAL_HEADER_BYTE_LEN + 4 && buffer[i++] == 'S' &&
      buffer[i++] == 'I' && buffer[i++] == 'X' &&
      (buffer[i] == 'T' || buffer[i] == 'U')) {
    return 1;
  }
...
int corsaro_flowtuple_init_input(corsaro_in_t *corsaro)
{
  ...
  if ((state = malloc_zero(sizeof(struct corsaro_flowtuple_in_state_t))) ==
      NULL) {
    corsaro_log_in(__func__, corsaro,
                   "could not malloc corsaro_flowtuple_state_t");
    goto err;
...
off_t corsaro_flowtuple_read_global_data_record(
                                   ...
                                   corsaro_interval_t *int_end)
{
  ...
  for (i = 0; i <= CORSARO_FLOWTUPLE_CLASS_MAX; i++) {
    ...
    if ((CORSARO_FILE_MODE(state->outfile) == CORSARO_FILE_MODE_BINARY &&
         binary_dump(corsaro, i) != 0) ||
        (CORSARO_FILE_MODE(state->outfile) == CORSARO_FILE_MODE_ASCII &&
                                     ...
                                     corsaro_packet_t *packet)
{
  ...
  if (corsaro_flowtuple_add_inc(STATE(corsaro)->st_hash[class], &t, 1) != 0) {
    corsaro_log(__func__, corsaro, "could not increment value for flowtuple");
    return -1;
  }
  return 0;
}
                                        ...
                                        corsaro_packet_state_t *state)
{
  ...
  if (corsaro_flowtuple_add_inc(
        STATE(corsaro)->st_hash[STATE(corsaro)->current_class], flowtuple,
...
int corsaro_flowtuple_process_flowtuple_class_start(
...
int corsaro_flowtuple_process_flowtuple_class_end(
...
{
  return corsaro_file_printf(corsaro, file, "START %s %" PRIu32 "\n",
                             class_names[class->class_type], class->count);
}
                                      ...
                                      corsaro_in_record_t *record)
{
  switch (record_type) {
  case CORSARO_IN_RECORD_TYPE_FLOWTUPLE_CLASS_START:
    return corsaro_flowtuple_class_start_fprint(
      corsaro, file, (corsaro_flowtuple_class_start_t *)record->buffer);
    break;

  case CORSARO_IN_RECORD_TYPE_FLOWTUPLE_CLASS_END:
    return corsaro_flowtuple_class_end_fprint(
      corsaro, file, (corsaro_flowtuple_class_end_t *)record->buffer);
    break;
