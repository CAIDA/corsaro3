 ...
 */
#define CORSARO_CONFICKERSCAN_SHIFT_AND_XOR(value)                             \
  h ^= (h << 5) + (h >> 27) + (value)

static inline khint32_t
corsaro_confickerscan_hash_func(corsaro_confickerscan_hash_key_t *t)
{
...
static inline khint32_t corsaro_confickerscan_intermediate_aggregate_hash_func(
...
#define corsaro_confickerscan_intermediate_aggregate_hash_eq(alpha, bravo)     \
  ((alpha)->as == (bravo)->as && (alpha)->src_ip == (bravo)->src_ip)

static inline khint32_t corsaro_confickerscan_aggregate_hash_func(
...
static inline khint32_t corsaro_confickerscan_how_many_ips_hash_func(
