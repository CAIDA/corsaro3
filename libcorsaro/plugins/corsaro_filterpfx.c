...
static void usage(corsaro_plugin_t *plugin)
{
  fprintf(stderr,
          ...
          "up to %d times\n",
          plugin->argv[0], MAX_COMMAND_LINE_PREFIXES);
}
...
static int parse_args(corsaro_t *corsaro)
{
  ...
  if (state->pfx_file != NULL && state->cmd_prefix_cnt > 0) {
    fprintf(stderr, "WARNING: both -f and -p used, all specified prefixes "
                    "will be used\n");
  }
...
static int add_prefix(corsaro_t *corsaro, char *pfx_str)
{
  ...
  if ((pfx = ascii2prefix(AF_INET, pfx_str)) == NULL) {
    ...
    corsaro_log(__func__, corsaro, "failed to parse prefix '%s'", pfx_str);
    assert(0);
  }
                           ...
                           uint32_t ip_addr)
{
  ...
  if ((node = patricia_search_best2(plugin_state->trie, &pfx, 1)) == NULL) {
...
int corsaro_filterpfx_init_output(corsaro_t *corsaro)
{
  ...
  if (state->pfx_file != NULL) {
    if ((file = corsaro_file_ropen(state->pfx_file)) == NULL) {
      corsaro_log(__func__, corsaro, "failed to open prefix file '%s'",
                  state->pfx_file);
    ...
    if (read_pfx_file(corsaro, file) != 0) {
      corsaro_log(__func__, corsaro, "failed to read prefix file '%s'",
                  state->pfx_file);
      goto err;
    }
...
off_t corsaro_filterpfx_read_global_data_record(
...
int corsaro_filterpfx_process_flowtuple_class_start(
...
int corsaro_filterpfx_process_flowtuple_class_end(
