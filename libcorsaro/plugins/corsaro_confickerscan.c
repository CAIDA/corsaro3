...
KHASH_INIT(confickerscan_hash, corsaro_confickerscan_hash_key_t*, corsaro_confickerscan_hash_value_t*, 1, corsaro_confickerscan_hash_func, corsaro_confickerscan_hash_eq)

KHASH_INIT(confickerscan_intermediate_aggregate_hash, corsaro_confickerscan_intermediate_aggregate_hash_key_t*, corsaro_confickerscan_intermediate_aggregate_hash_value_t*, 1, corsaro_confickerscan_intermediate_aggregate_hash_func, corsaro_confickerscan_intermediate_aggregate_hash_eq)

KHASH_INIT(confickerscan_aggregate_hash, corsaro_confickerscan_aggregate_hash_key_t*, corsaro_confickerscan_aggregate_hash_value_t*, 1, corsaro_confickerscan_aggregate_hash_func, corsaro_confickerscan_aggregate_hash_eq)

KHASH_INIT(confickerscan_how_many_ips_hash, corsaro_confickerscan_how_many_ips_hash_key_t*, corsaro_confickerscan_how_many_ips_hash_value_t*, 1, corsaro_confickerscan_how_many_ips_hash_func, corsaro_confickerscan_how_many_ips_hash_eq)
...
#define STATE(corsaro)						\
  (CORSARO_PLUGIN_STATE(corsaro, confickerscan, CORSARO_PLUGIN_ID_CONFICKERSCAN))
...
int corsaro_confickerscan_probe_magic(corsaro_in_t *corsaro, corsaro_file_in_t *file)
{
...
int corsaro_confickerscan_init_output(corsaro_t *corsaro)
{
  ...
  state->intermediate_aggregate=kh_init(confickerscan_intermediate_aggregate_hash);
...
void corsaro_confickerscan_intermediate_aggregate_free(corsaro_confickerscan_intermediate_aggregate_hash_key_t *t)
{
  free(t);
}

void corsaro_confickerscan_aggregate_free(corsaro_confickerscan_aggregate_hash_key_t *t)
{
  free(t);
}

void corsaro_confickerscan_how_many_ips_free(corsaro_confickerscan_how_many_ips_hash_key_t *t)
{
  free(t);
}
...
off_t corsaro_confickerscan_intermediate_aggregate_fprint(corsaro_t *corsaro, corsaro_file_t *file,
				   khiter_t khiter)
{
  assert(corsaro != NULL);
  assert(file != NULL);
...
off_t corsaro_confickerscan_aggregate_fprint(corsaro_t *corsaro, corsaro_file_t *file,
					     khiter_t khiter, khiter_t khiter2, uint32_t ts)
{

  assert(corsaro != NULL);
  assert(file != NULL);

  corsaro_confickerscan_aggregate_hash_value_t * v = kh_value(STATE(corsaro)->aggregate, khiter);
  corsaro_confickerscan_how_many_ips_hash_value_t * hmv = kh_value(STATE(corsaro)->hmips, khiter2);
  ...
  return corsaro_file_printf(corsaro, file,
			     ...
			     v->num_packets_distribution[0],
			     v->num_packets_distribution[1],
			     v->num_packets_distribution[2],
			     v->num_packets_distribution[3],
			     v->num_packets_distribution[4],
			     v->num_src_ips,
			     kh_size(h_dst));
...
int corsaro_confickerscan_aggregate_seen_ips(corsaro_t * corsaro, corsaro_confickerscan_hash_value_t * v)
{
  ...
  if((khiter = kh_get(confickerscan_how_many_ips_hash, STATE(corsaro)->hmips, &k)) == kh_end(STATE(corsaro)->hmips)){
    /* create a new key */
    if((new_key = malloc(sizeof(corsaro_confickerscan_how_many_ips_hash_key_t))) == NULL)
      {
    ...
    /* add it to the hash */
    khiter = kh_put(confickerscan_how_many_ips_hash, STATE(corsaro)->hmips, new_key, &khret);
...
int corsaro_confickerscan_intermediate_aggregate_flows(corsaro_t * corsaro, corsaro_confickerscan_hash_value_t * v)
{
  ...
  if((khiter = kh_get(confickerscan_intermediate_aggregate_hash, STATE(corsaro)->intermediate_aggregate, &ak)) == kh_end(STATE(corsaro)->intermediate_aggregate)){
    /* create a new key */
    if((new_key = malloc(sizeof(corsaro_confickerscan_intermediate_aggregate_hash_key_t))) == NULL)
      {
	corsaro_log_file(__func__, NULL, "malloc failed");
	return -1;
      }
    ...
    /* fill it */
    memcpy(new_key, &ak, sizeof(corsaro_confickerscan_intermediate_aggregate_hash_key_t));


    /* add it to the hash */
    khiter = kh_put(confickerscan_intermediate_aggregate_hash, STATE(corsaro)->intermediate_aggregate, new_key, &khret);
    ...
    kh_value(STATE(corsaro)->intermediate_aggregate, khiter)->total_packets=v->num_packets;
    for (int i=0; i<5; i++)
...
int corsaro_confickerscan_aggregate_flows(corsaro_t * corsaro, corsaro_confickerscan_intermediate_aggregate_hash_value_t * v)
{
  ...
  if((khiter = kh_get(confickerscan_aggregate_hash, STATE(corsaro)->aggregate, &ak)) == kh_end(STATE(corsaro)->aggregate)){
    /* create a new key */
    if((new_key = malloc(sizeof(corsaro_confickerscan_aggregate_hash_key_t))) == NULL)
      {
	corsaro_log_file(__func__, NULL, "malloc failed");
	return -1;
      }
    ...
    /* add it to the hash */
    khiter = kh_put(confickerscan_aggregate_hash, STATE(corsaro)->aggregate, new_key, &khret);
    ...
    kh_value(STATE(corsaro)->aggregate, khiter)->total_packets=v->total_packets;
    for (int i=0; i<5; i++)
...
int corsaro_confickerscan_close_output(corsaro_t *corsaro)
{
      ...
      if(state->outfile != NULL)
	{
	  ...
	  for (aik = kh_begin(state->intermediate_aggregate); aik != kh_end(state->intermediate_aggregate); ++aik)
	      ...
	      if (kh_exist(state->intermediate_aggregate, aik))
		{
		  ...
		  free(aiv);
		  kh_del(confickerscan_intermediate_aggregate_hash, state->intermediate_aggregate, aik);
		}
	    }
	  ...
	  for (ak = kh_begin(state->aggregate); ak != kh_end(state->aggregate); ++ak)
	      ...
	      if (kh_exist(state->aggregate, ak))
		{
		  ...
		  corsaro_confickerscan_aggregate_fprint(corsaro, state->outfile, ak, ip_k, -1);
		  free(av);
  ...
  kh_free(confickerscan_intermediate_aggregate_hash, state->intermediate_aggregate, &corsaro_confickerscan_intermediate_aggregate_free);
  kh_destroy(confickerscan_intermediate_aggregate_hash, state->intermediate_aggregate);
  state->intermediate_aggregate=NULL;

  kh_free(confickerscan_aggregate_hash, state->aggregate, &corsaro_confickerscan_aggregate_free);
			  ...
			  corsaro_in_record_t *record)
{
  ...
  corsaro_log_in(__func__, corsaro, "corsaro_confickerscan files are simply trace files."
...
int corsaro_confickerscan_start_interval(corsaro_t *corsaro, corsaro_interval_t *int_start)
{
  /* we don't care */
  return 0;
}

int corsaro_confickerscan_end_interval(corsaro_t *corsaro, corsaro_interval_t *int_end)
{
  struct corsaro_confickerscan_state_t *state = STATE(corsaro);
  khiter_t k, aik, ak, hmk;
  corsaro_confickerscan_hash_value_t * v;
  ...
  for (ak = kh_begin(state->aggregate); ak != kh_end(state->aggregate); ++ak)
      ...
      if (kh_exist(state->aggregate, ak))
	{
	  ...
	  hmipk.as=av->as;
	  ip_k =  kh_get(confickerscan_how_many_ips_hash, state->hmips, &hmipk);
	  corsaro_confickerscan_aggregate_fprint(corsaro, state->outfile, ak, ip_k, corsaro->interval_start.time);
	  free(av);
	  kh_del(confickerscan_aggregate_hash, state->aggregate, ak);
	}
    }
...
int corsaro_confickerscan_add_inc(kh_confickerscan_hash_t *h, corsaro_confickerscan_hash_key_t *k, corsaro_confickerscan_hash_value_t *v, corsaro_t *corsaro)
{
  int khret;
  khiter_t khiter;
  ...
  if((khiter = kh_get(confickerscan_hash, h, k)) == kh_end(h))
      ...
      if (!khret || khiter==kh_end(h))
	{
      ...
      /* create a new value */
      if((kh_value(h, khiter) = malloc(sizeof(corsaro_confickerscan_hash_value_t))) == NULL)
        {
          corsaro_log_file(__func__, NULL, "malloc failed");
          return -1;
        }
...
#ifdef CORSARO_CONFICKERSCAN_DEBUG
      //corsaro_log(__func__, NULL, "new flow: %d %d\n", kh_value(h,khiter)->as, kh_value(h,khiter)->num_packets);
#endif
    }
  else
    {
      /* test if we should expire based on time */
      float elapsed_sec = (v->last_timestamp.tv_sec-kh_value(h,khiter)->last_timestamp.tv_sec);
      elapsed_sec += ((float)(v->last_timestamp.tv_usec-kh_value(h,khiter)->last_timestamp.tv_usec))/1000000;

      if (elapsed_sec > CORSARO_CONFICKERSCAN_FLOW_TIMEOUT) {
	/* is old: store the old and copy in the new */
	corsaro_confickerscan_intermediate_aggregate_flows(corsaro, kh_value(h, khiter));
	/*corsaro_confickerscan_fprint(corsaro, STATE(corsaro)->outfile, khiter);*/
	memcpy(kh_value(h, khiter), v, sizeof(corsaro_confickerscan_hash_value_t));
      } else {
			     ...
			     corsaro_packet_t *packet)
{
  ...
  if (!(packet->state.os_class_id ==0 && strcmp(fp_os_names[packet->state.os_name_id], "Windows") == 0 &&  ((strcmp(packet->state.os_flavor, "XP") ==0) || strstr(packet->state.os_flavor, "NT")==0)))
    {
  ...
  if(ip_hdr->ip_p != TRACE_IPPROTO_TCP || !(ntohs(ip_hdr->ip_len) == 48  || ntohs(ip_hdr->ip_len) == 52))
    {
  ...
  if ((tcp_hdr == NULL)|| (!tcp_hdr->syn) || (tcp_hdr->fin) || (tcp_hdr->rst) || (tcp_hdr->psh) || (tcp_hdr->ack) || (tcp_hdr->urg) || (tcp_hdr->ece) || (tcp_hdr->cwr))
    {
  ...
  //corsaro_log(__func__, corsaro, "FLAGS: %x   confickerscan packet: (%s %s %d %d) with asnum %d and os (%d %s %s)", packet->state.flags, sip, dip, trace_get_source_port(ltpacket), trace_get_destination_port(ltpacket), packet->state.asn, packet->state.os_class_id, fp_os_names[packet->state.os_name_id], packet->state.os_flavor);
#endif
  ...
  if (corsaro_confickerscan_add_inc(STATE(corsaro)->st_hash, &k, &v, corsaro) != 0){
