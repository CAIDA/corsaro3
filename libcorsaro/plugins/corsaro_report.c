...
KHASH_INIT(u32metric, uint32_t, leafmetric_package_t *, 1, kh_int_hash_func2,
           kh_int_hash_equal)
...
typedef struct metric_tree {
  ...
  leafmetric_package_t
    *port_metrics[METRIC_PORT_PROTOCOL_MAX + 1][METRIC_PORT_DIRECTION_MAX + 1]
                 [METRIC_PORT_VAL_CNT];
...
#define METRIC_KEY_INIT(lmid)                                                  \
  do {                                                                         \
    snprintf(key_buffer, KEY_BUFFER_LEN, "%s.%s", metric_prefix,               \
             leafmetric_names[lmid]);                                          \
...
static leafmetric_package_t *
leafmetric_package_new(struct corsaro_report_state_t *state,
                       enum tree_id tree_id, enum submetric_id submetric_id,
                       const char *metric_prefix, uint32_t *id_offset)
{
...
static void metric_package_update(leafmetric_package_t *mp, uint32_t src_ip,
                                ...
                                leafmetric_package_t *mp)
{
  ...
  LMP_IF(LEAFMETRIC_FLAG_UNIQ_SRC_IP)
  {
    timeseries_kp_set(state->kp, mp->id_offset + offset,
  ...
  LMP_IF(LEAFMETRIC_FLAG_PKT_CNT)
  {
    timeseries_kp_set(state->kp, mp->id_offset + offset, mp->pkt_cnt);
  ...
  LMP_IF(LEAFMETRIC_FLAG_IP_LEN)
  {
    timeseries_kp_set(state->kp, mp->id_offset + offset, mp->ip_len);
...
#define METRIC_PREFIX_INIT(treeid, smid, target, prefix, format, instance)     \
  do {                                                                         \
    ...
    if ((target = leafmetric_package_new(state, treeid, smid, key_buffer,      \
                                         &key_id)) == NULL) {                  \
      goto err;                                                                \
    }                                                                          \
  } while (0)

#define SM_IF(smid) if ((tree_submetric_leafmetrics[tree->id][smid]) != 0)
                                      ...
                                      uint32_t *id_offset)
{
  ...
  if ((tree->group = corsaro_tag_group_init(corsaro, tree_names[tree_id],
                                            CORSARO_TAG_GROUP_MATCH_MODE_ALL,
                                            NULL)) == NULL) {
    corsaro_log(__func__, corsaro, "could not create group for %s",
                tree_names[tree_id]);
    goto err;
  }
  ...
  for (i = 0; i < state->tags_cnt; i++) {
    ...
    if ((tag_defs[i].tree_flags & tree_flags[tree_id]) != 0) {
      if (corsaro_tag_group_add_tag(tree->group, state->tags[i]) != 0) {
        corsaro_log(__func__, corsaro, "could not add tag %s to group %s",
  ...
  SM_IF(SUBMETRIC_ID_TREE)
  {
    ...
    snprintf(key_buffer, KEY_BUFFER_LEN, METRIC_PREFIX ".%s" METRIC_PATH_TREE,
             tree->group->name);
    if ((tree->tree_metrics = leafmetric_package_new(
           state, tree_id, SUBMETRIC_ID_TREE, key_buffer, &key_id)) == NULL) {
      goto err;
    }
  }
  ...
  SM_IF(SUBMETRIC_ID_FILTER)
  {
    ...
    for (i = 0; i < tags_cnt; i++) {
      METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_FILTER, tree->filter_metrics[i],
                         METRIC_PATH_FILTER, "%s", tags[i]->name);
    }
  }
  ...
  SM_IF(SUBMETRIC_ID_MAXMIND_CONTINENT)
  {
    ...
    for (i = 0; i < ARR_CNT(continent_strings); i++) {
      ...
      METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_MAXMIND_CONTINENT,
                         ...
                         METRIC_PATH_MAXMIND_CONTINENT, "%s",
                         continent_strings[i]);
    }
  }
  ...
  SM_IF(SUBMETRIC_ID_MAXMIND_COUNTRY)
  {
    ...
    for (i = 0; i < state->maxmind_countries_cnt; i++) {
      ...
      METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_MAXMIND_COUNTRY,
                         ...
                         METRIC_PATH_MAXMIND_COUNTRY, "%s", cc_str);
    }
  }

  if (tree_submetric_leafmetrics[tree_id][SUBMETRIC_ID_NETACQ_EDGE_CONTINENT] !=
        0 ||
      tree_submetric_leafmetrics[tree_id][SUBMETRIC_ID_NETACQ_EDGE_COUNTRY] !=
        0 ||
      tree_submetric_leafmetrics[tree_id][SUBMETRIC_ID_NETACQ_EDGE_REGION] !=
        0 ||
      tree_submetric_leafmetrics[tree_id][SUBMETRIC_ID_NETACQ_EDGE_POLYS] !=
        0) {
    /* get the netacq edge provider */
    if ((provider = corsaro_ipmeta_get_provider(
           corsaro, IPMETA_PROVIDER_NETACQ_EDGE)) == NULL ||
    ...
    if ((records_cnt = ipmeta_provider_get_all_records(provider, &records)) ==
        0) {
      corsaro_log(__func__, corsaro,
    ...
    SM_IF(SUBMETRIC_ID_NETACQ_EDGE_CONTINENT)
    {
      ...
      for (i = 0; i < ARR_CNT(continent_strings); i++) {
        ...
        METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_NETACQ_EDGE_CONTINENT,
                           ...
                           METRIC_PATH_NETACQ_EDGE_CONTINENT, "%s",
      ...
      *cc_ptr = '.';
      ...
      SM_IF(SUBMETRIC_ID_NETACQ_EDGE_COUNTRY)
      {
        METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_NETACQ_EDGE_COUNTRY,
                           ...
                           METRIC_PATH_NETACQ_EDGE_COUNTRY, "%s", cc_str);
      }
    }
    ...
    SM_IF(SUBMETRIC_ID_NETACQ_EDGE_REGION)
    {
      ...
      for (i = 0; i < regions_cnt; i++) {
        ...
        METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_NETACQ_EDGE_REGION,
                           ...
                           METRIC_PATH_NETACQ_EDGE_REGION, "%s", rc_str);
      }
    }
    ...
    SM_IF(SUBMETRIC_ID_NETACQ_EDGE_POLYS)
    {
      tree->netacq_poly_tbl_cnt =
        ipmeta_provider_netacq_edge_get_polygon_tables(provider, &poly_tbls);
      ...
      for (i = 0; i < tree->netacq_poly_tbl_cnt; i++) {
        ...
        for (j = 0; j < table->polygons_cnt; j++) {
          if (table->polygons[j]->id + 1 > tree->netacq_poly_metrics_cnt[i]) {
            tree->netacq_poly_metrics_cnt[i] = table->polygons[j]->id + 1;
          }
        }
        ...
        for (j = 0; j < table->polygons_cnt; j++) {
          ...
          METRIC_PREFIX_INIT(
            tree_id, SUBMETRIC_ID_NETACQ_EDGE_POLYS,
            tree->netacq_poly_metrics[i][table->polygons[j]->id],
            METRIC_PATH_NETACQ_EDGE_POLYS, "%s", table->polygons[j]->fqid);
        }
      }
    }
  ...
  SM_IF(SUBMETRIC_ID_PFX2AS)
  {
    ...
    if ((provider = corsaro_ipmeta_get_provider(
           corsaro, IPMETA_PROVIDER_PFX2AS)) == NULL ||
        ipmeta_is_provider_enabled(provider) == 0) {
      corsaro_log(__func__, corsaro,
    ...
    if ((pfx2as_records_cnt =
           ipmeta_provider_get_all_records(provider, &pfx2as_records)) <= 0) {
      corsaro_log(__func__, corsaro,
    ...
    ks_introsort(pfx2as_ip_cnt_desc, pfx2as_records_cnt, pfx2as_records);
    ...
    for (i = 0; i < pfx2as_records_cnt &&
      ...
      METRIC_PREFIX_INIT(tree_id, SUBMETRIC_ID_PFX2AS, tmp_mp,
                         METRIC_PATH_PFX2AS, "%" PRIu32, tmp_asn);
    ...
    for (proto = 0; proto <= METRIC_PORT_PROTOCOL_MAX; proto++) {
      for (dir = 0; dir <= METRIC_PORT_DIRECTION_MAX; dir++) {
        for (port = 0; port < tree_port_max[tree_id]; port++) {
          ...
          METRIC_PREFIX_INIT(
            tree_id, SUBMETRIC_ID_PORT, tree->port_metrics[proto][dir][port],
  ...
  SM_IF(SUBMETRIC_ID_PFX2AS)
  {
    if (tree->pfx2as_metrics != NULL) {
      for (khiter = kh_begin(tree->pfx2as_metrics);
           khiter != kh_end(tree->pfx2as_metrics); ++khiter) {
        if (kh_exist(tree->pfx2as_metrics, khiter)) {
          metric_package_destroy(kh_val(tree->pfx2as_metrics, khiter));
        }
      }
      ...
      for (dir = 0; dir <= METRIC_PORT_DIRECTION_MAX; dir++) {
        for (port = 0; port < tree_port_max[tree->id]; port++) {
          if (tree->port_metrics[proto][dir][port] != NULL) {
            metric_package_destroy(tree->port_metrics[proto][dir][port]);
                            ...
                            enum tree_id tree_id)
{
  ...
  SM_IF(SUBMETRIC_ID_NETACQ_EDGE_POLYS)
  {
    for (j = 0; j < tree->netacq_poly_tbl_cnt; j++) {
      DUMP_ARRAY(tree->netacq_poly_metrics[j], tree->netacq_poly_metrics_cnt[j])
    }
  }
  ...
  SM_IF(SUBMETRIC_ID_PFX2AS)
  {
    for (khiter = kh_begin(tree->pfx2as_metrics);
         khiter != kh_end(tree->pfx2as_metrics); ++khiter) {
      if (kh_exist(tree->pfx2as_metrics, khiter)) {
        metric_package_dump(state, kh_val(tree->pfx2as_metrics, khiter));
      }
    }
  }
  ...
  SM_IF(SUBMETRIC_ID_PORT)
  {
    for (proto = 0; proto <= METRIC_PORT_PROTOCOL_MAX; proto++) {
      for (dir = 0; dir <= METRIC_PORT_DIRECTION_MAX; dir++) {
        for (port = 0; port < tree_port_max[tree->id]; port++) {
          if (tree->port_metrics[proto][dir][port] != NULL) {
            metric_package_dump(state, tree->port_metrics[proto][dir][port]);
          }
        }
      }
    }
...
static inline uint16_t lookup_convert_cc(corsaro_packet_state_t *state,
                                         ...
                                         uint16_t def, uint16_t *cont)
{
  ipmeta_record_t *record;

  if ((record = corsaro_ipmeta_get_record(state, provider_id)) != NULL) {
...
static int process_generic(corsaro_t *corsaro, corsaro_packet_state_t *state,
                           ...
                           uint16_t dst_port, uint16_t ip_len, uint8_t protocol,
                           uint64_t pkt_cnt)
{
  ...
  maxmind_cc =
    lookup_convert_cc(state, IPMETA_PROVIDER_MAXMIND, 0x3F3F, /* "??" */
                      &maxmind_cont);
  /* netacq edge country code */
  netacq_cc =
    lookup_convert_cc(state, IPMETA_PROVIDER_NETACQ_EDGE, 0x3F3F, /* "??" */
                      &netacq_cont);
  /* netacq edge region code */
  if ((record = corsaro_ipmeta_get_record(
         state, IPMETA_PROVIDER_NETACQ_EDGE)) != NULL) {
  ...
  for (tree_idx = 0; tree_idx < TREE_ID_CNT; tree_idx++) {
    ...
    SM_IF(SUBMETRIC_ID_FILTER)
    {
      ...
      for (j = 0; j < tags_cnt; j++) {
        ...
        if (corsaro_tag_is_match(state, tags[j]) == 0) {
          metric_package_update(tree->filter_metrics[j], src_ip, dst_ip, ip_len,
                                pkt_cnt);
        }
      }
    }
    ...
    if (corsaro_tag_group_is_match(state, tree->group) != 0) {
      ...
      SM_IF(SUBMETRIC_ID_TREE)
      {
        metric_package_update(tree->tree_metrics, src_ip, dst_ip, ip_len,
                              pkt_cnt);
      }
      ...
      SM_IF(SUBMETRIC_ID_MAXMIND_COUNTRY)
      {
        ...
        metric_package_update(tree->maxmind_country_metrics[maxmind_cc], src_ip,
                              dst_ip, ip_len, pkt_cnt);
      }
      ...
      SM_IF(SUBMETRIC_ID_NETACQ_EDGE_COUNTRY)
      {
        ...
        metric_package_update(tree->netacq_country_metrics[netacq_cc], src_ip,
                              dst_ip, ip_len, pkt_cnt);
      }
      ...
      SM_IF(SUBMETRIC_ID_NETACQ_EDGE_REGION)
      {
        if (netacq_rc != 0) {
          ...
          metric_package_update(tree->netacq_region_metrics[netacq_rc], src_ip,
                                dst_ip, ip_len, pkt_cnt);
        }
      }
      ...
      SM_IF(SUBMETRIC_ID_NETACQ_EDGE_POLYS)
      {
        ...
        for (j = 0; j < netacq_poly_ids_cnt; j++) {
          if (tree->netacq_poly_metrics[j][netacq_poly_ids[j]] == NULL) {
            corsaro_log(__func__, corsaro, "Missing region polygon %d:%d. ", j,
                        netacq_poly_ids[j]);
            assert(0); /* in case we have asserts on */
            return -1;
          }
          metric_package_update(
            tree->netacq_poly_metrics[j][netacq_poly_ids[j]], src_ip, dst_ip,
            ip_len, pkt_cnt);
        }
      }
      ...
      SM_IF(SUBMETRIC_ID_PFX2AS)
      {
        ...
        if ((record = corsaro_ipmeta_get_record(
               state, IPMETA_PROVIDER_PFX2AS)) != NULL &&
            record->asn_cnt == 1 &&
          ...
          metric_package_update(kh_val(tree->pfx2as_metrics, khiter), src_ip,
                                dst_ip, ip_len, pkt_cnt);
        }
      }
      ...
      SM_IF(SUBMETRIC_ID_PROTOCOL)
      {
        metric_package_update(tree->protocol_metrics[protocol], src_ip, dst_ip,
                              ip_len, pkt_cnt);
      }
      ...
      SM_IF(SUBMETRIC_ID_PORT)
      {
        ...
        if (proto != METRIC_PORT_PROTOCOL_SKIP) {
          if (src_port < tree_port_max[tree->id]) {
            metric_package_update(
              tree->port_metrics[proto][METRIC_PORT_DIRECTION_SRC][src_port],
              src_ip, dst_ip, ip_len, pkt_cnt);
          }

          if (dst_port < tree_port_max[tree->id]) {
            metric_package_update(
              tree->port_metrics[proto][METRIC_PORT_DIRECTION_DST][dst_port],
              src_ip, dst_ip, ip_len, pkt_cnt);
          }
        }
      }
...
static void usage(corsaro_t *corsaro)
{
  ...
  fprintf(stderr, "usage: %s -b backend [-b backend]\n"
...
static int parse_args(corsaro_t *corsaro)
{
  ...
  if (backends_cnt == 0) {
    fprintf(stderr, "ERROR: At least one backend must be specified using -b\n");
    usage(corsaro);
    goto err;
  }
...
int corsaro_report_init_output(corsaro_t *corsaro)
{
  ...
  if ((state = malloc_zero(sizeof(struct corsaro_report_state_t))) == NULL) {
    corsaro_log(__func__, corsaro, "could not malloc corsaro_report_state_t");
    goto err;
  }
  ...
  if ((state->timeseries = timeseries_init()) == NULL) {
    corsaro_log(__func__, corsaro, "could not initialize libtimeseries");
    goto err;
  }
  ...
  if ((state->kp = timeseries_kp_init(state->timeseries, 0)) == NULL) {
    corsaro_log(__func__, corsaro, "could not create key package");
    goto err;
  }
  ...
  for (i = 0; i < ARR_CNT(tag_defs); i++) {
    ...
    if ((state->tags[i] =
           corsaro_tag_init(corsaro, tag_defs[i].name, bpf_filter)) == NULL) {
  ...
  if ((provider = corsaro_ipmeta_get_provider(
         corsaro, IPMETA_PROVIDER_MAXMIND)) != NULL &&
      ipmeta_is_provider_enabled(provider) != 0) {
  ...
  for (i = 0; i < TREE_ID_CNT; i++) {
    if (state->trees_enabled == 0 ||
      ...
      if ((state->trees[i] = metric_tree_new(corsaro, i, &key_id)) == NULL) {
        goto err;
      }
...
off_t corsaro_report_read_global_data_record(
...
int corsaro_report_end_interval(corsaro_t *corsaro, corsaro_interval_t *int_end)
{
  struct corsaro_report_state_t *state = STATE(corsaro);
  int i;
...
int corsaro_report_process_packet(corsaro_t *corsaro, corsaro_packet_t *packet)
{
  struct corsaro_report_state_t *state = STATE(corsaro);
  ...
  for (i = 0; i < state->tags_cnt; i++) {
    assert(state->tags[i] != NULL);
    if (state->tags[i]->user != NULL) {
      rc = trace_apply_filter((libtrace_filter_t *)(state->tags[i]->user),
                              ltpacket);
      if (rc < 0) {
  ...
  if (process_generic(corsaro, &packet->state, ip_hdr->ip_src.s_addr,
                      ip_hdr->ip_dst.s_addr, src_port, dst_port,
                      ntohs(ip_hdr->ip_len), ip_hdr->ip_p, 1) != 0) {
    return -1;
  }
