...
static void usage(corsaro_plugin_t *plugin)
{
  fprintf(
    ...
    plugin->argv[0], MAX_COMMAND_LINE_COUNTRIES);
}
...
static int parse_args(corsaro_t *corsaro)
{
  ...
  if (state->country_file != NULL && state->cmd_country_cnt > 0) {
    fprintf(stderr, "WARNING: both -f and -c used, all specified countries "
                    "will be used\n");
  }
...
static int add_country(corsaro_t *corsaro, char *cc_str)
{
  ...
  if (strnlen(cc_str, BUFFER_LEN) != 2) {
    corsaro_log(__func__, corsaro, "Invalid country code %s", cc_str);
    return -1;
  }
...
int corsaro_filtergeo_init_output(corsaro_t *corsaro)
{
  ...
  if (state->country_file != NULL) {
    if ((file = corsaro_file_ropen(state->country_file)) == NULL) {
      corsaro_log(__func__, corsaro, "failed to open country file '%s'",
                  state->country_file);
    ...
    if (read_country_file(corsaro, file) != 0) {
      corsaro_log(__func__, corsaro, "failed to read country file '%s'",
                  state->country_file);
      goto err;
    }
...
off_t corsaro_filtergeo_read_global_data_record(
...
int corsaro_filtergeo_process_flowtuple_class_start(
...
int corsaro_filtergeo_process_flowtuple_class_end(
