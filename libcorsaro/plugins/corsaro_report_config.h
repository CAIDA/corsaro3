...
const char *leafmetric_names[] = {"uniq_src_ip", "uniq_dst_ip", "pkt_cnt",
                                  "ip_len"};
...
const uint8_t tree_flags[] = {
  TREE_FLAG_UNFILTERED, TREE_FLAG_NONSPOOFED, TREE_FLAG_NONERRATIC,
  TREE_FLAG_RFCCLEAN,
};

const char *tree_names[] = {
  "unfiltered", "non-spoofed", "non-erratic", "rfc5735-non-spoofed",
};
...
const uint8_t tree_submetric_leafmetrics[TREE_ID_CNT][SUBMETRIC_ID_CNT] = {
  ...
  {
    /* Maxmind continent */
    LEAFMETRIC_FLAG_UNIQ_SRC_IP | LEAFMETRIC_FLAG_UNIQ_DST_IP |
    ...
    /* Tree */
    LEAFMETRIC_FLAG_UNIQ_SRC_IP | LEAFMETRIC_FLAG_UNIQ_DST_IP |
      LEAFMETRIC_FLAG_PKT_CNT | LEAFMETRIC_FLAG_IP_LEN,
  },
...
static char *port_metric_paths[] = {
  PORT_PREFIX ".tcp.src_port", PORT_PREFIX ".tcp.dst_port",
  PORT_PREFIX ".udp.src_port", PORT_PREFIX ".udp.dst_port",
  /*
...
const struct tag_def tag_defs[] = {
  /* unfiltered tag */
  {
    "all-pkts", TREE_FLAG_UNFILTERED, NULL,
  },
  ...
  {
    "ttl-200", TREE_FLAG_NONSPOOFED | TREE_FLAG_NONERRATIC | TREE_FLAG_RFCCLEAN,
    "((ip[8] < 200) or icmp)",
  },
  {
  ...
  {
    "rfc5735", TREE_FLAG_NONSPOOFED | TREE_FLAG_NONERRATIC,
  ...
  {
    "backscatter", TREE_FLAG_NONERRATIC,
    "not (tcp and ((tcp[tcpflags] & tcp-ack != 0) and (tcp[tcpflags] & tcp-syn "
    "!= 0) or (tcp[tcpflags] & tcp-rst !=0))) and not (icmp and (icmp[0]=0 || "
    "icmp[0]=3 || icmp[0]=4 || icmp[0]=5 || icmp[0]=11 || icmp[0]=12 || "
    "icmp[0]=14 || icmp[0]=16 || icmp[0]=18)) and not (udp src port 53)",
  },
  {
    "bittorrent", TREE_FLAG_NONERRATIC,
    "not (udp and udp [4:2] >= 20  and ((udp[8:4]=0x64313a61 or "
    "udp[8:4]=0x64313a72) and udp[12:4]=0x64323a69 and udp[16:4]=0x6432303a)) "
    "and not (udp[4:2]>= 44 and udp and udp[28:4]=0x13426974 and "
    "udp[32:4]=0x546f7272  and udp[36:4]=0x656e7420 and udp[40:4]=0x70726f74 "
    "and udp[44:4]=0x6f636f6c) and not (ip[2:2]>=0x3a and (udp[8:2]=0x4102 or "
    "udp[8:2]=0x2102 or udp[8:2]=0x3102 or udp[8:2]=0x1102) and "
    "udp[udp[4:2]-4:4]=0 and udp[udp[4:2]-8:4]=0 and "
    "udp[udp[4:2]-10:2]=0x0008) and not (ip[2:2]=0x30 and (udp[8:2]=0x4100 or "
    "udp[8:2]=0x2100 or udp[8:2]=0x3102 or udp[8:2]=0x1100)) and not (udp and "
    "ip[2:2]=61 and udp[20:4]=0x7fffffff and udp[24:4]=0xab020400 and "
    "udp[28:4]=0x01000000 and udp[32:4]=0x08000000 and udp[36:4]=0)",
  },
  {"udp-0x31", TREE_FLAG_NONERRATIC, "not (udp and ip[2:2]=58 and udp[8:4]=0x0 "
                                     "and udp[12:4]=0x00 and "
                                     "udp[16:2]=0x3100)"},
  {"udp-ip-len-96", TREE_FLAG_NONERRATIC, "not (udp and ip[2:2]=96)"},
  {
    "port-53", TREE_FLAG_NONERRATIC, "not port 53",
  },
  {
    "tcp-port-23", TREE_FLAG_NONERRATIC, "not tcp port 23",
  },
  {
    "tcp-port-80", TREE_FLAG_NONERRATIC, "not tcp port 80",
  },
  {
    "tcp-port-5000", TREE_FLAG_NONERRATIC, "not tcp port 5000",
  },
  {
    "dns-resp-non-standard", /* DISABLED */
    0, "not (udp and ip[2:2]> 0x2a and udp[10:2] & 0xfff0=0x8180 and "
       "udp[12:2]=0001 and udp[14:4]=0x00000000 and udp[18:2]=0x0000)",
  },
  {
    "dns-resp-non-standard-v2", TREE_FLAG_NONERRATIC,
    "not (udp and ip[2:2]> 0x2a and udp[10:2] & 0xfff0=0x8180 and udp[12:2] < "
    "10 and udp[14:2]< 10 and udp[16:2] < 10 and udp[18:2] < 10)",
  },
  {
    "netbios-query-name", TREE_FLAG_NONERRATIC,
    "not (udp and src port 137 and dst port 137 and ip[2:2]> 0x30 and "
    "udp[20:2]=0x2043 and udp[22:2]=0x4b41 and udp[24:4]=0x41414141)",
  },

  /* rfcclean only tags */
  {
    "not-rfc5735", TREE_FLAG_RFCCLEAN,
    "(src net 0.0.0.0/8 or src net 10.0.0.0/8 or src net 127.0.0.0/8 or src "
    "net 169.254.0.0/16 or src net 172.16.0.0/12 or src net 192.0.0.0/24 or "
    "src net 192.0.2.0/24 or src net 192.88.99.0/24 or src net 192.168.0.0/16 "
    "or src net 198.18.0.0/15 or src net 198.51.100.0/24 or src net "
    "203.0.113.0/24 or src net 224.0.0.0/4 or src net 240.0.0.0/4)",
  },
