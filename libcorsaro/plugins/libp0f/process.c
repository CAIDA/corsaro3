...
static void find_offset(const u8 *data, s32 total_len)
{
  ...
  for (i = 0; i < 40; i += 2, total_len -= 2) {
    ...
    if (total_len >= MIN_TCP6 && (data[i] >> 4) == IP_VER6) {
      ...
      if (hdr->proto == PROTO_TCP) {
        ...
        DEBUG_P0F("[#] Detected packet offset of %u via IPv6 (link type %u).\n",
                  i, link_type);
        link_off = i;
        break;
      }
    ...
    if ((data[i] >> 4) == IP_VER4) {
      ...
      if (hdr->proto == PROTO_TCP) {
        ...
        DEBUG_P0F("[#] Detected packet offset of %u via IPv4 (link type %u).\n",
                  i, link_type);
        link_off = i;
        break;
      }
  ...
  if (link_off >= 4 && data[i - 4] == 0x81 && data[i - 3] == 0x00) {
    ...
    DEBUG_P0F("[#] Adjusting offset due to VLAN tagging.\n");
    link_off -= 4;
...
u8 *addr_to_str(u8 *data, u8 ip_ver)
{
  ...
  if (ip_ver == IP_VER4) {
    ...
    sprintf(tmp, "%x:%x:%x:%x:%x:%x:%x:%x", (data[0] << 8) | data[1],
...
void parse_packet(void *junk, const struct pcap_pkthdr *hdr, const u8 *data)
{
  ...
  packet_cnt++;
  if (!(packet_cnt % EXPIRE_INTERVAL))
    expire_cache();
  parse_packet_helper(&pk, hdr, data, 1);
  flow_dispatch(&pk);
}

void parse_packet_helper(struct packet_data *pk, const struct pcap_pkthdr *hdr,
                         const u8 *data, u8 link_headers)
{
  struct tcp_hdr *tcp;
  ...
  // DEBUG_P0F("[#] Received packet: len = %d, caplen = %d, limit = %d\n",
  ...
  if (packet_len < MIN_TCP4) {
    DEBUG_P0F("[#] Packet too short for any IPv4 + TCP headers, giving up!\n");
    return;
  }
  ...
  if ((*data >> 4) == IP_VER4) {
    ...
    if (packet_len > tot_len) {
      packet_len = tot_len;
      // DEBUG_P0F("[#] ipv4.tot_len = %u, adjusted accordingly.\n", tot_len);
    }

    /* Bail out if the result leaves no room for IPv4 + TCP headers. */

    if (packet_len < MIN_TCP4) {
      DEBUG_P0F("[#] packet_len = %u. Too short for IPv4 + TCP, giving up!\n",
                packet_len);
      return;
    }
    ...
    if (hdr_len < sizeof(struct ipv4_hdr)) {
      DEBUG_P0F("[#] ipv4.hdr_len = %u. Too short for IPv4, giving up!\n",
                hdr_len);
      return;
    }
    ...
    if (tot_len > packet_len) {
      DEBUG_P0F("[#] ipv4.tot_len = %u but packet_len = %u, bailing out!\n",
                tot_len, packet_len);
      return;
    }
    ...
    if (hdr_len + sizeof(struct tcp_hdr) > packet_len) {
      DEBUG_P0F("[#] ipv4.hdr_len = %u, packet_len = %d, no room for TCP!\n",
                hdr_len, packet_len);
      return;
    }
    ...
    if (ip4->proto != PROTO_TCP) {
      DEBUG_P0F("[#] Whoa, IPv4 packet with non-TCP payload (%u)?\n",
                ip4->proto);
      return;
    }
    ...
    if (flags_off & ~(IP4_DF | IP4_MBZ)) {
      DEBUG_P0F("[#] Packet fragment (0x%04x), letting it slide!\n", flags_off);
      return;
    }
    ...
    if (packet_len > tot_len) {
      packet_len = tot_len;
      // DEBUG_P0F("[#] ipv6.tot_len = %u, adjusted accordingly.\n", tot_len);
    }

    /* Bail out if the result leaves no room for IPv6 + TCP headers. */

    if (packet_len < MIN_TCP6) {
      DEBUG_P0F("[#] packet_len = %u. Too short for IPv6 + TCP, giving up!\n",
                packet_len);
      return;
    }
    ...
    if (tot_len > packet_len) {
      DEBUG_P0F("[#] ipv6.tot_len = %u but packet_len = %u, bailing out!\n",
                tot_len, packet_len);
      return;
    }
    ...
    if (ip6->proto != PROTO_TCP) {
      DEBUG_P0F("[#] IPv6 packet with non-TCP payload (%u).\n", ip6->proto);
      return;
    }
  ...
  if (tcp_doff < sizeof(struct tcp_hdr)) {
    DEBUG_P0F("[#] tcp.hdr_len = %u, not enough for TCP!\n", tcp_doff);
    return;
  }

  if (tcp_doff > packet_len) {
    DEBUG_P0F("[#] tcp.hdr_len = %u, past end of packet!\n", tcp_doff);
    return;
  }
  ...
  if (((tcp->flags & TCP_SYN) && (tcp->flags & (TCP_FIN | TCP_RST))) ||
      ((tcp->flags & TCP_FIN) && (tcp->flags & TCP_RST)) || !pk->tcp_type) {

    DEBUG_P0F("[#] Silly combination of TCP flags: 0x%02x.\n", tcp->flags);
    return;
  }
  ...
  if (tcp->flags & TCP_ACK) {
    ...
    if (RD32(tcp->ack) & !(tcp->flags & TCP_RST)) {
      ...
      DEBUG_P0F("[#] Non-zero ACK on a non-ACK packet: 0x%08x.\n",
                ntohl(RD32(tcp->ack)));
  ...
  if (tcp->flags & TCP_URG) {
    ...
    if (RD16(tcp->urg)) {
      ...
      DEBUG_P0F("[#] Non-zero UPtr on a non-URG packet: 0x%08x.\n",
                ntohl(RD16(tcp->urg)));
  ...
  while (data < opt_end && pk->opt_cnt < MAX_TCP_OPT) {
    ...
    case TCPOPT_MAXSEG:
      ...
      if (*data != 4) {
        DEBUG_P0F("[#] MSS option expected to have 4 bytes, not %u.\n", *data);
        pk->quirks |= QUIRK_OPT_BAD;
      }

      if (data + 3 > opt_end) {
        DEBUG_P0F("[#] MSS option would end past end of header (%u left).\n",
                  opt_end - data);
        goto abort_options;
      }
    ...
    case TCPOPT_WSCALE:
      ...
      if (*data != 3) {
        DEBUG_P0F("[#] MSS option expected to have 3 bytes, not %u.\n", *data);
        pk->quirks |= QUIRK_OPT_BAD;
      }

      if (data + 2 > opt_end) {
        DEBUG_P0F("[#] WS option would end past end of header (%u left).\n",
                  opt_end - data);
        goto abort_options;
      }
    ...
    case TCPOPT_SACKOK:
      ...
      if (*data != 2) {
        DEBUG_P0F("[#] SACKOK option expected to have 2 bytes, not %u.\n",
                  *data);
        pk->quirks |= QUIRK_OPT_BAD;
      }

      if (data + 1 > opt_end) {
        DEBUG_P0F("[#] SACKOK option would end past end of header (%u left).\n",
                  opt_end - data);
        goto abort_options;
      }
    ...
    case TCPOPT_SACK:
      ...
      if (*data < 10 || *data > 34) {
        DEBUG_P0F("[#] SACK length out of range (%u), bailing out.\n", *data);
        goto abort_options;
      }

      if (data - 1 + *data > opt_end) {
        DEBUG_P0F("[#] SACK option (len %u) is too long (%u left).\n", *data,
                  opt_end - data);
        goto abort_options;
      }
    ...
    case TCPOPT_TSTAMP:
      ...
      if (*data != 10) {
        DEBUG_P0F("[#] TStamp option expected to have 10 bytes, not %u.\n",
                  *data);
        pk->quirks |= QUIRK_OPT_BAD;
      }

      if (data + 9 > opt_end) {
        DEBUG_P0F("[#] TStamp option would end past end of header (%u left).\n",
                  opt_end - data);
        goto abort_options;
      }
      ...
      if (pk->tcp_type == TCP_SYN && RD32p(data + 5)) {
        ...
        DEBUG_P0F("[#] Non-zero second timestamp: 0x%08x.\n",
                  ntohl(*(u32 *)(data + 5)));
    ...
    default:
      ...
      if (*data < 2 || *data > 40) {
        DEBUG_P0F("[#] Unknown option 0x%02x has invalid length %u.\n",
                  data[-1], *data);
        goto abort_options;
      }

      if (data - 1 + *data > opt_end) {
        DEBUG_P0F("[#] Unknown option 0x%02x (len %u) is too long (%u left).\n",
  ...
  abort_options:
    ...
    DEBUG_P0F("[#] Option parsing aborted (cnt = %u, remainder = %u).\n",
...
static void destroy_host(struct host_data *h)
{
  ...
  DEBUG_P0F("[#] Destroying host data: %s (bucket %d)\n",
...
static struct host_data *create_host(u8 *addr, u8 ip_ver)
{
  ...
  DEBUG_P0F("[#] Creating host data: %s (bucket %u)\n",
...
static void touch_host(struct host_data *h)
{
  ...
  DEBUG_P0F("[#] Refreshing host data: %s\n", addr_to_str(h->addr, h->ip_ver));
...
static void nuke_flows(u8 silent)
{
  ...
  if (silent)
    DEBUG_P0F("[#] Pruning connections - trying to delete %u...\n", kcnt);
  else
...
static struct packet_flow *create_flow_from_syn(struct packet_data *pk)
{
  ...
  DEBUG_P0F("[#] Creating flow from SYN: %s/%u -> ",
            addr_to_str(pk->src, pk->ip_ver), pk->sport);

  DEBUG_P0F("%s/%u (bucket %u)\n", addr_to_str(pk->dst, pk->ip_ver), pk->dport,
            bucket);
...
static void expire_cache(void)
{
  ...
  DEBUG_P0F("[#] Cache expiration kicks in...\n");
...
static void flow_dispatch(struct packet_data *pk)
{
  ...
  DEBUG_P0F("[#] Received TCP packet: %s/%u -> ",
            addr_to_str(pk->src, pk->ip_ver), pk->sport);
  ...
  case TCP_SYN:
    ...
    if (f) {
      ...
      DEBUG_P0F("[#] New SYN for an existing flow, resetting.\n");
      destroy_flow(f);
    }
  ...
  case TCP_SYN | TCP_ACK:
    ...
    if (!f) {
      ...
      DEBUG_P0F("[#] Stray SYN+ACK with no flow.\n");
      return;
    }
    ...
    if (to_srv) {
      ...
      DEBUG_P0F("[#] SYN+ACK from client to server, trippy.\n");
      return;
    }
    ...
    if (f->acked) {
      ...
      if (f->next_srv_seq - 1 != pk->seq)
        DEBUG_P0F(
          "[#] Repeated but non-identical SYN+ACK (0x%08x != 0x%08x).\n",
  ...
  case TCP_ACK:
    ...
    if (!f->acked) {
      ...
      DEBUG_P0F("[#] Never received SYN+ACK to complete handshake, huh.\n");
      destroy_flow(f);
      return;
    }
    ...
    if (!need_more) {
      ...
      DEBUG_P0F("[#] All modules done, no need to keep tracking flow.\n");
      destroy_flow(f);

    } else if (f->req_len >= MAX_FLOW_DATA && f->resp_len >= MAX_FLOW_DATA) {

      DEBUG_P0F("[#] Per-flow capture size limit exceeded.\n");
      destroy_flow(f);
    }
  ...
  default:
    ...
    WARN("Huh. Unexpected packet type 0x%02x in flow_dispatch().",
         pk->tcp_type);
  }
}
...
void verify_tool_class(u8 to_srv, struct packet_flow *f, u32 *sys, u32 sys_cnt)
{
  ...
  if (i == sys_cnt) {
    ...
    DEBUG_P0F("[#] Detected app not supposed to run on host OS.\n");
    add_nat_score(to_srv, f, NAT_APP_SIG, 4);

  } else {

    DEBUG_P0F("[#] Detected app supported on host OS.\n");
