...
static s32 lookup_hdr(u8 *name, u32 len, u8 create)
{
  ...
  while (i--) {
    if (!memcmp(hdr_names[*p], name, len) && !hdr_names[*p][len])
      return *p;
    p++;
  }
  ...
  hdr_by_hash[bucket] =
    DFL_ck_realloc(hdr_by_hash[bucket], (hbh_cnt[bucket] + 1) * 4);
...
static void http_find_match(u8 to_srv, struct http_sig *ts, u8 dupe_det)
{
  ...
  while (cnt--) {
    ...
    while (rs_hdr < rs->hdr_cnt) {
      ...
      while (rs->hdr[rs_hdr].id != ts->hdr[ts_hdr].id && ts_hdr < ts->hdr_cnt)
        ts_hdr++;
      ...
      if (rs->hdr[rs_hdr].value && (!ts->hdr[ts_hdr].value ||
                                    !strstr((char *)ts->hdr[ts_hdr].value,
                                            (char *)rs->hdr[rs_hdr].value)))
        goto next_sig;
  ...
  if (!dupe_det && gmatch) {
    ...
    if (gmatch->sig->sw && ts->sw &&
        !strstr((char *)ts->sw, (char *)gmatch->sig->sw))
      ts->dishonest = 1;
  }
}
                       ...
                       u8 *val, u32 line_no)
{
  ...
  sigs[to_srv] = DFL_ck_realloc(
    sigs[to_srv], sizeof(struct http_sig_record) * (sig_cnt[to_srv] + 1));
  ...
  if (hsig->matched)
    FATAL("Signature in line %u is already covered by line %u.", line_no,
...
void http_parse_ua(u8 *val, u32 line_no)
{
  ...
  while (*val) {
    ...
    ua_map =
      DFL_ck_realloc(ua_map, (ua_map_cnt + 1) * sizeof(struct ua_map_record));
...
static void score_nat(u8 to_srv, struct packet_flow *f)
{
  ...
  if (!m) {
    ...
    if (!to_srv && ref && ref->matched) {
      ...
      DEBUG_P0F("[#] HTTP server signature changed from known to unknown.\n");
      score += 4;
      reason |= NAT_TO_UNK;
    }
  ...
  if (m->class_id == -1) {
    ...
    if (!to_srv && ref && ref->matched) {
      ...
      if (ref->matched->name_id != m->name_id) {
        ...
        DEBUG_P0F("[#] Name on the matched HTTP server signature changes.\n");
        score += 8;
        reason |= NAT_APP_LB;

      } else if (ref->matched->label_id != m->label_id) {

        DEBUG_P0F("[#] Label on the matched HTTP server signature changes.\n");
        score += 2;
        reason |= NAT_APP_LB;
      }
    }
    ...
    if (ref && ref->matched) {
      ...
      if (ref->matched->name_id != m->name_id) {
        ...
        DEBUG_P0F("[#] Name on the matched HTTP OS signature changes.\n");
        score += 8;
        reason |= NAT_OS_SIG;
        diff_already = 1;
        ...
        DEBUG_P0F("[#] Label on the matched HTTP OS signature changes.\n");
        score += 2;
        reason |= NAT_OS_SIG;
      }
      ...
      DEBUG_P0F("[#] HTTP OS signature changed from unknown to known.\n");
      score += 4;
      reason |= NAT_TO_UNK;
    }
    ...
    if (!diff_already && hd->last_name_id != m->name_id) {
      ...
      DEBUG_P0F("[#] Matched HTTP OS signature different than host data.\n");
      score += 4;
      reason |= NAT_OS_SIG;
    }
  }
  ...
  if (to_srv && m->class_id == -1 && f->http_tmp.sw && !f->http_tmp.dishonest) {
    ...
    for (i = 0; i < ua_map_cnt; i++)
      if (strstr((char *)f->http_tmp.sw, (char *)ua_map[i].name))
        break;
    ...
    if (i != ua_map_cnt) {
      ...
      if (ua_map[i].id != hd->last_name_id) {
        ...
        DEBUG_P0F("[#] Otherwise plausible User-Agent points to another OS.\n");
        score += 4;
        reason |= NAT_APP_UA;
        ...
        DEBUG_P0F("[#] User-Agent OS value checks out.\n");
        hd->bad_sw = 0;
      }
    }
  }
...
header_check:
  ...
  if (f->http_tmp.via) {
    DEBUG_P0F("[#] Explicit use of Via or X-Forwarded-For.\n");
    score += 8;
    reason |= NAT_APP_VIA;
  }
  ...
  if (ref && !to_srv && ref->date && f->http_tmp.date) {
    ...
    if (hdr_diff < -HTTP_MAX_DATE_DIFF ||
      ...
      DEBUG_P0F("[#] HTTP 'Date' distance seems fine (%lld in %lld sec).\n",
                hdr_diff, recv_diff);
    }
  }
...
static void fingerprint_http(u8 to_srv, struct packet_flow *f)
{
  ...
  if ((m = f->http_tmp.matched)) {
    ...
    OBSERVF((m->class_id < 0) ? "app" : "os", "%s%s%s", fp_os_names[m->name_id],
            m->flavor ? " " : "", m->flavor ? m->flavor : (u8 *)"");
  ...
  if (f->http_tmp.lang && isalpha(f->http_tmp.lang[0]) &&
    ...
    while (languages[lh][pos]) {
      if (f->http_tmp.lang[0] == languages[lh][pos][0] &&
          f->http_tmp.lang[1] == languages[lh][pos][1])
        break;
      pos += 2;
    }
    ...
    if (!languages[lh][pos])
      ...
      add_observation_field("lang", (lang = (u8 *)languages[lh][pos + 1]));
  ...
  if (!to_srv) {
    ...
    if (m) {
      ...
      if (m->class_id != -1) {
        ...
        f->client->http_req_os =
          ck_memdup(&f->http_tmp, sizeof(struct http_sig));
...
void free_sig_hdrs(struct http_sig *h)
{
  ...
  for (i = 0; i < h->hdr_cnt; i++) {
    if (h->hdr[i].name)
      ck_free(h->hdr[i].name);
    if (h->hdr[i].value)
      ck_free(h->hdr[i].value);
  }
}
...
static u32 parse_date(u8 *str)
{
  ...
  if (!strptime((char *)str, "%a, %d %b %Y %H:%M:%S %Z", &t)) {
    DEBUG_P0F("[#] Invalid 'Date' field ('%s').\n", str);
    return 0;
  }
...
static u8 parse_pairs(u8 to_srv, struct packet_flow *f, u8 can_get_more)
{
  ...
  while ((off = f->http_pos) < plen) {
    ...
    if ((hcount = f->http_tmp.hdr_cnt) >= HTTP_MAX_HDRS) {
      ...
      DEBUG_P0F("[#] Too many HTTP headers in a %s.\n",
    ...
    if (off == plen) {
      ...
      if (!can_get_more) {
        ...
        DEBUG_P0F("[#] End of HTTP %s before end of headers.\n",
                  to_srv ? "request" : "response");
        f->in_http = -1;
      }
    ...
    if (!nlen || pay[off] != ':' || nlen > HTTP_MAX_HDR_NAME) {
      ...
      DEBUG_P0F(
        "[#] Invalid HTTP header encountered (len = %u, char = 0x%02x).\n",
        nlen, pay[off]);
    ...
    if (vlen > HTTP_MAX_HDR_VAL) {
      DEBUG_P0F("[#] HTTP %s header value length exceeded.\n",
                to_srv ? "request" : "response");
      f->in_http = -1;
      return -1;
    }
  ...
  if (to_srv) {
    ...
    if (!f->in_http) {
      ...
      if (!off && strncmp((char *)pay, "GET /", 5) &&
        ...
        DEBUG_P0F("[#] Does not seem like a GET / HEAD request.\n");
        f->in_http = -1;
        return 0;
      }
      ...
      while (off < f->req_len && off < HTTP_MAX_URL &&
        ...
        if (chr != '\r' && (chr < 0x20 || chr > 0x7f)) {
          ...
          DEBUG_P0F("[#] Not HTTP - character 0x%02x encountered.\n", chr);
      ...
      if (off == HTTP_MAX_URL || off < 14) {
        ...
        DEBUG_P0F("[#] Not HTTP - newline offset %u.\n", off);
      ...
      if (off == f->req_len) {
        ...
        if (!can_get_more) {
          ...
          DEBUG_P0F("[#] Not HTTP - no opening line found.\n");
          f->in_http = -1;
        }
      ...
      if (strncmp((char *)sig_at, "HTTP/1.", 7)) {
        ...
        DEBUG_P0F("[#] Not HTTP - bad signature.\n");
    ...
    if (!f->http_gotresp1) {
      ...
      while (off < f->resp_len && off < HTTP_MAX_URL &&
        ...
        if (chr != '\r' && (chr < 0x20 || chr > 0x7f)) {
          ...
          DEBUG_P0F(
            "[#] Invalid HTTP response - character 0x%02x encountered.\n", chr);
          f->in_http = -1;
          return 0;
        }
      ...
      if (off == HTTP_MAX_URL || off < 13) {
        ...
        DEBUG_P0F("[#] Invalid HTTP response - newline offset %u.\n", off);
      ...
      if (off == f->resp_len) {
        ...
        if (!can_get_more) {
          ...
          DEBUG_P0F("[#] Invalid HTTP response - no opening line found.\n");
          f->in_http = -1;
        }
      ...
      if (strncmp((char *)pay, "HTTP/1.", 7)) {
        ...
        DEBUG_P0F("[#] Invalid HTTP response - bad signature.\n");
      ...
      DEBUG_P0F("[#] HTTP response starts correctly.\n");
    }
