...
static u32 regen_pfds(struct pollfd* pfds, struct api_client** ctable) {
  ...
  DEBUG_P0F("[#] Recomputing pollfd data, pcap_fd = %d.\n", pfds[0].fd);
    ...
    for (cur = 0; cur < pfd_count; cur++) {
        ...
        case 1:
          ...
          if (pfd_count - 2 < api_max_conn) {
            ...
            if (api_cl[i].fd < 0) {
              ...
              DEBUG_P0F("[#] Accepted new API connection, fd %d.\n", api_cl[i].fd);
      ...
      if (pfds[cur].revents & (POLLERR | POLLHUP)) switch (cur) {
        ...
        default:
          ...
          DEBUG_P0F("[#] API connection on fd %d closed.\n", pfds[cur].fd);
