...
static u32 class_cnt, /* Sizes for maps                     */
  name_cnt, label_id, /* Current label ID                   */
...
static void config_parse_classes(u8 *val)
{
  ...
  while (*val) {
    ...
    fp_os_classes =
      DFL_ck_realloc(fp_os_classes, (class_cnt + 1) * sizeof(u8 *));
...
static void config_parse_label(u8 *val)
{
  ...
  if (*val == '!' && val[1] == ':') {
    ...
    *nxt = 0;
    ...
    for (i = 0; i < class_cnt; i++)
      if (!strcasecmp((char *)val, (char *)fp_os_classes[i]))
        break;
  ...
  if (nxt == val || *nxt != ':')
    FATAL("Malformed name in line %u.", line_no);
  ...
  while (*val) {
    ...
    *nxt = 0;
    ...
    if (is_cl) {
      ...
      for (i = 0; i < name_cnt; i++)
        if (!strcasecmp((char *)val, (char *)fp_os_names[i]))
          break;

      if (i == name_cnt) {
        fp_os_names =
          DFL_ck_realloc(fp_os_names, (name_cnt + 1) * sizeof(u8 *));
...
static void config_parse_line(u8 *line)
{
  ...
  if (*line == '[') {
    ...
    if (!dir)
      FATAL("Malformed section identifier in line %u.", line_no);
    ...
    *dir = 0;
    ...
    if (!strcmp((char *)line, "tcp")) {
      ...
      FATAL("Unrecognized fingerprinting module '%s' in line %u.", line,
            line_no);
    }
  ...
  *eon = 0;
  ...
  if (!strcmp((char *)line, "classes")) {
    ...
    if (state != CF_NEED_SIG)
      FATAL("Misplaced 'sig' in line %u.", line_no);
