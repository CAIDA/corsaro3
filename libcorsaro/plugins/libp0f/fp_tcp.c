...
#define RET_IF_DIV(_div, _use_mtu, _desc)                                      \
  do {                                                                         \
    if ((_div) && !(win % (_div))) {                                           \
      *use_mtu = (_use_mtu);                                                   \
      DEBUG_P0F("[#] Window size %u is a multiple of %s [%u].\n", win, _desc,  \
                _div);                                                         \
      return win / (_div);                                                     \
    }                                                                          \
  } while (0)
  ...
  if (ts->ip_ver == IP_VER6)
    RET_IF_DIV(mss + MIN_TCP6, 1, "MTU (IPv6)");
                      ...
                      u8 *val, u32 line_no)
{
  ...
  sigs[to_srv][bucket] = DFL_ck_realloc(sigs[to_srv][bucket],
                                        (sig_cnt[to_srv][bucket] + 1) *
                                          sizeof(struct tcp_sig_record));
...
static void score_nat(u8 to_srv, struct tcp_sig *sig, struct packet_flow *f)
{
  ...
  if (!ref) {
    ...
    if (sig->matched && hd->last_class_id != -1) {
      ...
      if (hd->last_name_id != sig->matched->name_id) {
        ...
        DEBUG_P0F("[#] New TCP signature different OS type than host data.\n");
  ...
  if (!sig->matched || !ref->matched) {
    ...
    if ((sig->quirks ^ ref->quirks) &
        ~(QUIRK_ECN | QUIRK_DF | QUIRK_NZ_ID | QUIRK_ZERO_ID)) {

      DEBUG_P0F("[#] Non-fuzzy quirks changed compared to previous sig.\n");
      ...
      DEBUG_P0F("[#] SYN option layout changed compared to previous sig.\n");
    ...
    if (to_srv && sig->matched != ref->matched) {
      ...
      DEBUG_P0F("[#] SYN signature changed from %s.\n",
    ...
    if (ref->matched->name_id != sig->matched->name_id) {
      ...
      DEBUG_P0F("[#] TCP signature different OS type on previous sig.\n");
      score += 8;
      reason |= NAT_OS_SIG;
      ...
      if (ref->matched->label_id != sig->matched->label_id) {
        ...
        DEBUG_P0F("[#] SYN signature label different on previous sig.\n");
        score += 2;
        reason |= NAT_OS_SIG;
        ...
        DEBUG_P0F("[#] SYN signature changes within the same label.\n");
        score += 1;
        reason |= NAT_OS_SIG;
        ...
        DEBUG_P0F("[#] SYN signature fuzziness changes.\n");
        score += 1;
        reason |= NAT_FUZZY;
      }
    }
  ...
  if (!diff_already && sig->matched && hd->last_class_id != -1 &&
    ...
    DEBUG_P0F("[#] New OS signature different OS type than host data.\n");
    score += 8;
    reason |= NAT_OS_SIG;
    diff_already = 1;
  }
...
#define ABS(_x) ((_x) < 0 ? -(_x) : (_x))
  ...
  if (!diff_already && ttl_diff && (!sig->matched || !sig->matched->bad_ttl) &&
      (!ref->matched || !ref->matched->bad_ttl) &&
      (sig->dist <= NEAR_TTL_LIMIT || ttl_diff > 1)) {

    DEBUG_P0F("[#] Signature TTL differs by %d (dist = %u).\n", ttl_diff,
              sig->dist);
  ...
  if (to_srv && hd->last_port && f->cli_port < hd->last_port &&
    ...
    DEBUG_P0F("[#] Source port drops from %u to %u.\n", hd->last_port,
              f->cli_port);
  ...
  if (sig->mss != ref->mss) {
    ...
    DEBUG_P0F("[#] MSS for signature changed from %u to %u.\n", ref->mss,
              sig->mss);
  ...
  if (score && sig->ts1 && ref->ts1) {
    ...
    if (ms_diff < MAX_NAT_TS) {
      ...
      if (ts_diff > max_ts && (ms_diff >= TSTAMP_GRACE || ~ts_diff > max_ts)) {
        ...
        DEBUG_P0F("[#] Dodgy timestamp progression across signatures (%d "
                  "in %llu ms).\n",
                  ts_diff, ms_diff);
        score += 4;
        reason |= NAT_TS;
      ...
      DEBUG_P0F("[#] Timestamps available, but with bad interval (%llu ms).\n",
                ms_diff);
  }
...
struct tcp_sig *fingerprint_tcp_simple(u8 to_srv, struct packet_data *pk)
{
...
void check_ts_tcp(u8 to_srv, struct packet_data *pk, struct packet_flow *f)
{
  ...
  if (ts_diff < 5 ||
      (ms_diff < TSTAMP_GRACE && (~ts_diff) / 1000 < MAX_TSCALE / TSTAMP_GRACE))
    return;
  ...
  if (ffreq < MIN_TSCALE || ffreq > MAX_TSCALE) {
    ...
    DEBUG_P0F("[#] Bad %s TS frequency: %.02f Hz (%d ticks in %llu ms).\n",
  ...
  OBSERVF("uptime", "%u days %u hrs %u min (modulo %u days)",
          (up_min / 60 / 24), (up_min / 60) % 24, up_min % 60, up_mod_days);
