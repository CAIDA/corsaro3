...
void parse_packet_helper(struct packet_data *pk, const struct pcap_pkthdr *hdr, const u8* data, u8 link_headers){
  struct tcp_hdr* tcp;
    ...
    case TCP_SYN | TCP_ACK:
      ...
      if (f->acked) {
        ...
        if (f->next_srv_seq - 1 != pk->seq)
          DEBUG_P0F("[#] Repeated but non-identical SYN+ACK (0x%08x != 0x%08x).\n",
