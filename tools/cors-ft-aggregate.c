...
static char *field_names[] = {
  ...
  "ttl",    "tcp_flags", "ip_len",   "packet_cnt",
};
...
static corsaro_interval_t last_dump_end = {CORSARO_MAGIC,
                                           CORSARO_MAGIC_INTERVAL, 0, 0};

/** The time that we need to dump the next interval at */
static int next_interval = 0;
/** The time that the last interval ended */
static corsaro_interval_t last_interval_end = {CORSARO_MAGIC,
                                               CORSARO_MAGIC_INTERVAL, 0, 0};
 ...
 */
int add_inc_hash(kh_sixt_int_t *hash, corsaro_flowtuple_t *t,
                 uint32_t increment)
{
  int khret;
  khiter_t khiter;
...
int process_flowtuple_file(char *file)
{
  ...
  while ((len = corsaro_in_read_record(corsaro, &type, record)) > 0) {
    ...
    if (type == CORSARO_IN_RECORD_TYPE_IO_INTERVAL_START) {
      interval_record =
        (corsaro_interval_t *)corsaro_in_get_record_data(record);
...
static void usage(const char *name)
{
  fprintf(
    stderr,
    "usage: %s [-l] [-i interval] [-v value_field] [-f field]... [-F "
    "file_list] \n"
    "          flowtuple_file [flowtuple_file]\n"
    "       -l             treat the input files as containing legacy format "
    "data\n"
    ...
    "       -v <value>     field to use as aggregation value (default: "
    "packet_cnt)\n"
